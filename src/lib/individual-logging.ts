...
export const individualSignalSchema = z.object({
  sleep_quality: z.union([z.number().min(1).max(5), z.string().min(1)]),
  energy_level: z.union([z.number().min(1).max(5), z.string().min(1)]),
  stress_level: z.union([z.number().min(1).max(5), z.string().min(1)]),
  recovery_feel: z.union([z.number().min(1).max(5), z.string().min(1)]),
  soreness_level: z.union([z.number().min(1).max(5), z.string().min(1)]),
  session_completion: z.enum(['missed', 'partial', 'complete', 'crushed']),
  training_minutes: z.number().min(0).max(300),
  session_rpe: z.number().min(0).max(10),
  steps: z.number().min(0).max(100000),
  hydration_liters: z.number().min(0).max(10),
  heat_level: z.union([z.enum(['normal', 'warm', 'hot', 'extreme']), z.literal('')]).optional().default(''),
  humidity_level: z.union([z.enum(['low', 'moderate', 'high']), z.literal('')]).optional().default(''),
  aqi_band: z.union([z.enum(['good', 'moderate', 'poor', 'very_poor']), z.literal('')]).optional().default(''),
  commute_minutes: z.number().min(0).max(240).optional().default(0),
  exam_stress_score: z.number().min(0).max(5).optional().default(0),
  fasting_state: z.union([z.enum(['none', 'light', 'strict']), z.literal('')]).optional().default(''),
  shift_work: z.boolean().optional().default(false),
  session_notes: z.string().max(300).optional().default(''),
})

export type IndividualSignalPayload = z.infer<typeof individualSignalSchema>
...
export type IndividualSignalResult =
  | { error: string }
  | {
      success: true
      result: {
...
export async function logIndividualSignalForUser(args: {
  supabase: SupabaseLike
  userId: string
  payload: IndividualSignalPayload
}): Promise<IndividualSignalResult> {
  ...
  const { data: healthRows, error: healthRowsError } = await selectRecentHealthMetrics(supabase, userId, 14)
  ...
  const readinessHistory = (historyRows || [])
    .map((row: Record<string, unknown>) => Number(row.readiness_score))
    ...
    .upsert(
      {
        ...
        alert_priority:
          recomputed.currentState.readinessScore < 45
            ? 'Critical'
            : recomputed.currentState.readinessScore < 60
              ? 'Warning'
              : 'Informational',
