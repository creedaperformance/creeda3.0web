...
export const athleteDailyCheckInSchema = z.object({
...
export type AthleteDailyCheckInInput = z.infer<typeof athleteDailyCheckInSchema>

export type AthleteDailyCheckInResult =
...
export async function submitAthleteDailyCheckInForUser(args: {
  supabase: SupabaseLike
  userId: string
  parsed: AthleteDailyCheckInInput
}): Promise<AthleteDailyCheckInResult> {
  const { supabase, userId, parsed } = args
  const logDate = getTodayInIndia()
  const context = await getAthleteDecisionContext(supabase, userId, { beforeDate: logDate })
  ...
  const decisionContext = contextSignal
    ? {
        ...context,
  ...
  const currentLog = buildCurrentLog(userId, logDate, parsed)
  ...
  const actionInstruction =
    result.creedaDecision.components.training.focus ||
    result.creedaDecision.sessionType ||
    'Follow today’s protocol.'
  const primaryReason =
    result.creedaDecision.explanation.primaryDrivers[0]?.reason ||
    result.decision.message ||
    'Daily decision generated.'
  const alertPriority =
...
function buildCurrentLog(userId: string, logDate: string, parsed: AthleteDailyCheckInInput) {
  const sessionCompleted =
    parsed.sessionCompletion === 'completed' || parsed.sessionCompletion === 'competition'
...
async function upsertAdaptationProfileCompat(
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const result = await supabase.from('adaptation_profiles').upsert(candidatePayload, { onConflict: 'user_id' })
...
function mapWellnessToTen(
  value:
    | AthleteDailyCheckInInput['sleepQuality']
    | AthleteDailyCheckInInput['energyLevel']
    | AthleteDailyCheckInInput['muscleSoreness']
    | AthleteDailyCheckInInput['lifeStress'],
