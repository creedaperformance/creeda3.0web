...
function averageCompliance(history: Array<{ compliancePct: number | null }>) {
...
export default async function IndividualPlansPage() {
  ...
  const [todaySession, weeklyCalendar, history, exerciseHistory] = await Promise.all([
    getOrCreateTodayExecutionSessionForIndividual(supabase, user.id),
    buildWeeklyCalendarForIndividual(supabase, user.id),
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-col gap-4 rounded-[30px] border border-white/8 bg-white/[0.03] p-5 shadow-[0_30px_80px_rgba(0,0,0,0.35)] md:flex-row md:items-end md:justify-between">
          <div className="space-y-3">
            ...
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight">
                Personal plan that turns guidance into action
              </h1>
              <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-300">
                Today&apos;s session, weekly rhythm, missed-session carry forward, recovery days, and exercise progression all live in the individual journey now.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              <Link href="/individual/exercises">
            ...
            <div className="rounded-[30px] border border-white/8 bg-white/[0.03] p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-black tracking-tight">
                    Actual execution, not just suggested work
                  </h2>
                </div>
                    ...
                    >
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        ...
                        <MiniMetric
                          label="Best volume"
                          value={
                            entry.bestRepVolume !== null
                              ? `${Math.round(entry.bestRepVolume)}`
                              : 'N/A'
                          }
                        />
                      </div>
            ...
            <div className="rounded-[30px] border border-white/8 bg-white/[0.03] p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-black tracking-tight">
                    Keep recovery and why-this-plan together
                  </h2>
                </div>
              </div>
              ...
              <div className="mt-5 rounded-[24px] border border-[var(--saffron)]/20 bg-[var(--saffron)]/8 p-4">
                <p className="text-sm leading-relaxed text-slate-100">
                  Recovery blocks are already injected into today&apos;s session when readiness is low or when rehab focus areas are active. You do not have to go hunt for cooldowns or rehab work anymore.
                </p>
              </div>

              <div className="mt-4 space-y-3">
                {todaySession.session.explainability.reasons.slice(0, 3).map((reason) => (
                  <div
                    key={reason}
                    className="rounded-[22px] border border-white/8 bg-black/20 p-4"
                  >
                    <p className="text-sm leading-relaxed text-white">{reason}</p>
                  </div>
                ))}
              </div>
