...
import { allExerciseLibraryItems, queryExercises } from '@/lib/product'
import {
  EXERCISE_CATEGORIES,
  SESSION_BLOCK_TYPES,
  SUPPORTED_SPORTS,
...
import { getOrCreateTodayExecutionSessionForIndividual } from '@/lib/product/server'
...
function filterHref(key: string, value: string | null, params: SearchParams | undefined) {
  ...
  return query ? `/individual/exercises?${query}` : '/individual/exercises'
}

function selectedFilters(params: SearchParams | undefined) {
  return {
    search: getParam(params, 'search') || undefined,
    category: fromAllowed(EXERCISE_CATEGORIES, getParam(params, 'category')) as
      | ExerciseCategory
      | undefined,
    blockType: fromAllowed(SESSION_BLOCK_TYPES, getParam(params, 'blockType')) as
      | SessionBlockType
      | undefined,
    sport: fromAllowed(SUPPORTED_SPORTS, getParam(params, 'sport')) as
      | SupportedSport
      | undefined,
...
async function getTodaySessionPreview(
  ...
  try {
    return await getOrCreateTodayExecutionSessionForIndividual(supabase, userId)
  } catch {
    return null
  }
}
...
export default async function IndividualExercisesPage(props: {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] px-4 pb-24 pt-16 text-white md:pl-72 md:pr-6 md:pt-6">
      <RoleDesktopNav role="individual" />
      ...
      <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">
                Browse the movements behind your personal plan
              </h1>
              <p className="mt-3 text-sm leading-relaxed text-slate-300">
                Warmup, main work, cooldown, recovery, rehab, and goal-specific movements are now reachable from the individual journey too.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              >
                <Link href="/individual/sessions/today">
              ...
              >
                <Link href="/individual/plans">Plan calendar</Link>
              </Button>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <StatCard
            ...
            helper={
              todaySession
                ? `${todaySession.expectedDurationMinutes} min plan`
              ...
              {personalizedExercises.map((exercise) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-slate-300">
                    {exercise.explanation[0] ||
                      'Selected for today based on your readiness, goal, and available setup.'}
                  </p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-semibold text-slate-300">
                      {exercise.prescribed.reps ||
                        `${exercise.prescribed.durationSeconds || 0}s`}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <section className="rounded-[30px] border border-white/8 bg-white/[0.03] p-5">
          ...
          <form
            className="mt-5 grid gap-3 lg:grid-cols-[1fr_180px_180px_180px_120px]"
            action="/individual/exercises"
          >
            ...
            <div className="rounded-[30px] border border-dashed border-white/10 bg-white/[0.02] p-10 text-center">
              ...
              <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                <Button
                  ...
                  className="h-12 rounded-2xl bg-[var(--saffron)] px-6 text-sm font-black text-black hover:brightness-110"
                >
                  <Link href="/individual/sessions/today">Open today&apos;s session</Link>
                </Button>
                <Button
                  asChild
                  variant="outline"
                  className="h-12 rounded-2xl border-white/12 bg-transparent px-6 text-sm font-bold text-white hover:bg-white/5"
                >
                  <Link href="/individual/exercises">Clear filters</Link>
                </Button>
              </div>
            </div>
          )}
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <MiniSpec
            label="Reps / Time"
            value={
              exercise.defaultPrescription.reps ||
              `${exercise.defaultPrescription.durationSeconds || 0}s`
            }
          />
        </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-black/20 p-4">
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-300">
            {exercise.coachingCues[0] ||
              ...
              'Move with control and stop if pain changes sharply.'}
          </p>
        </div>
