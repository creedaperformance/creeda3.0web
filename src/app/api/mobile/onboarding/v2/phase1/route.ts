...
import { OnboardingV2Phase1SubmissionSchema } from '@creeda/schemas'

import { authenticateMobileApiRequest } from '@/lib/mobile/auth'
import { persistOnboardingV2Phase1 } from '@/lib/onboarding-v2/persistence'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = OnboardingV2Phase1SubmissionSchema.safeParse({
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Invalid onboarding v2 Phase 1 payload.', details: parsed.error.flatten() },
      { status: 400 }
    )
  }
