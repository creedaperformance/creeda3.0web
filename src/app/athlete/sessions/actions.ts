...
export async function completeExecutionSession(input: z.infer<typeof completeSessionSchema>) {
  ...
  try {
    ...
    await trackProductEvent(supabase, {
      ...
      surface: 'athlete_guided_session',
      properties: {
    ...
    const message = error instanceof Error ? error.message : 'Unable to save your session right now.'
